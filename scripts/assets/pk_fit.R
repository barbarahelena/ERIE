...
#'      own fit - in this project's data, squared-residual scale differed
#'      ~24x between the top and bottom quartile of predicted concentration
#'      (see docs/pk-model.md), which under-weights the tail even after
#'      cross-curve normalization. `floor` is `weight_floor_frac` times that
#'      curve's own observed Cmax.
#'
...
#' solutions that look fine only at the sparse observed points. Both are
#' smooth (a squared shortfall/excess, zero at and below the threshold) so
#' neither introduces a discontinuity for the optimizer's finite-difference
#' gradient to trip over:
...
#'     without the hard, discontinuous penalty a literal floor would add)
...
build_joint_objective <- function(curves, min_tmax = NULL, cmax_tol = NULL,
                                   cmax_lambda = 20, tmax_lambda = 50,
...
#' as a poor fit (low R^2) or a search that didn't converge. `retry_mask`
#' is therefore caller-supplied, not inferred from `bound_cols` alone, so
#' callers can (and should) include poor-fit and non-convergence flags
#' alongside boundary flags, not boundary flags only.
#'
#' Retries every row flagged by `bound_cols`, `extra_flag_cols`, or
#' `convergence_col` (any of them, not boundary flags alone - see above),
#' and replaces that row only if the retry's `objective_value` is strictly
#' lower - so this can only improve `results`, never make it worse, however
#' many rows are retried. A caller supplying only `bound_cols` still works
#' (boundary-only retry), but should also pass its poor-fit/non-convergence
#' columns where it has them, for the reason above.
#'
