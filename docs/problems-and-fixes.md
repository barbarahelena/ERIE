...
Vd was first a flat 0.15 L/kg (an extracellular fluid volume estimate from glucose literature), then the individual Nadler blood volume from weight, height and sex, and is now the individual extracellular fluid volume (Watson total body water / 3) from weight, height, age and sex. Blood volume (about 65 to 75 mL/kg) is much smaller than the ECFV (about 150 to 200 mL/kg), so the move from the flat 0.15 L/kg to Nadler made `F_12C` and `F_13C6` smaller. In an earlier run under the Nadler Vd the median `F_12C` was about 3.2% and the median `F_13C6` about 3.0%; the last Nadler run gave medians of about 1.9%. The move to the Watson ECF goes the other way: the mean ECF is about 2.7 times the mean Nadler blood volume in this cohort, and `F` scales with `Vd`, so `F` from the three versions cannot be compared with each other. `ka` and `kel` are unaffected by any of these changes. The argument for ECFV (Vd of glucose equals the extracellular space, van der Crabben et al. 2011) was not refuted by the Nadler version, and the choice of the ECF has not been checked against a fructose-specific Vd.
