...
- **Lost to follow-up in the diet summary (2026-09-24).** ER33 and ER34 have only a baseline visit. They stay in the per-subject CSVs (`diet_curve_metrics.csv`, `diet_two_wave_params.csv`) with `completed = FALSE`, and every plot and statistic uses only subjects who completed both visits, so the comparison only involves subjects who took part in both visits.
- **Curve-metric plot titles (2026-09-24).** The AUC, Cmax and Tmax boxplots had titles such as "AUC 0-inf (mg*min/L) by diet arm and visit (from fitted curves, paired Wilcoxon)". The title is now the metric name and the unit is on the y axis.
